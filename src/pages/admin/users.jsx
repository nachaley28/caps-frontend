    ...
    <div className="container mt-4">
      ...
      <div className="row mb-3 g-2">
        ...
        <div className="col-md-3">
          <select className="form-select shadow-sm" value={filterRole} onChange={e => setFilterRole(e.target.value)}>
          ...
          <tbody>
            {filteredUsers.length === 0 ? (
              ...
              <tr key={idx} className="table-row-hover">
                ...
                <td>{user.email}</td>
                <td>
                  <span className={`badge bg-${user.role === 'Admin' ? 'danger' : user.role === 'Lab Assistant' ? 'success' : 'secondary'}`}>
                    {user.role}
                  </span>
                </td>
                <td>
                ...
                <td className="d-flex gap-1">
                  <button className="btn btn-sm btn-primary" title="View User" onClick={() => handleViewUser(user)}><FaEye /></button>
                  <button className="btn btn-sm btn-warning" title="Edit User" onClick={() => handleEditUser(user)}><FaEdit /></button>
                  <button className="btn btn-sm btn-danger" title="Delete User" onClick={() => handleDeleteUser(user)}><FaTrash /></button>
                  <button className={`btn btn-sm ${user.status === 'Active' ? 'btn-secondary' : 'btn-success'}`} 
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content shadow">
              <div className="modal-header">
                <h5 className="modal-title">{modalType === "view" ? "View User" : modalType === "edit" ? "Edit User" : "Add User"}</h5>
                ...
                {modalType === "view" ? (
                  ...
                  <form onSubmit={handleFormSubmit}>
                    <div className="mb-3">
                      ...
                      <input type="text" className="form-control" value={selectedUser.name} onChange={e => setSelectedUser({...selectedUser, name: e.target.value})} required />
                    </div>
                    <div className="mb-3">
                    ...
                    <div className="mb-3">
                      ...
                      <select className="form-select" value={selectedUser.role} onChange={e => setSelectedUser({...selectedUser, role: e.target.value})} required>
                    ...
                    <div className="mb-3">
                      ...
                      <select className="form-select" value={selectedUser.status} onChange={e => setSelectedUser({...selectedUser, status: e.target.value})}>
                        <option value="Active">Active</option>
                        <option value="Inactive">Inactive</option>
                      </select>
                    </div>
                    <button type="submit" className="btn btn-primary">{modalType === "edit" ? "Update" : "Add"}</button>
                  </form>
                )}
              </div>
            </div>
